...
import com.example.mealme.mapper.MealFileMapper;
...
public class MealFileService {
    private final MealFileMapper mealFileMapper;
    ...
    public void register(BoardFileDto boardfileDto){
        if(boardfileDto == null) { throw new IllegalArgumentException("파일 정보 누락"); }
        mealFileMapper.insert(boardfileDto);
    }
    ...
    public void remove(Long boardNumber){
        ...
        mealFileMapper.delete(boardNumber);
    }

    public List<BoardFileDto> findList(Long boardNumber){
        return mealFileMapper.selectList(boardNumber);
    }
...
//        boardNumber를 제외한 모든 정보를 가진 FileDto를 반환한다.
        ...
        boardFileDto.setFileUuid(uuid.toString());
        boardFileDto.setFileName(originName);
        boardFileDto.setFileUploadPath(getUploadPath());
    ...
    public List<BoardFileDto> findOldList(){
        return mealFileMapper.selectOldList();
    }
