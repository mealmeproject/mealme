import * as register from "../module/registChange.js";
...
function getCategoryCode1() {
  return fetch(`/api/getCategoryCode1?productNumber=${productNum}`)
      .then(response => {
        if (!response.ok) {
          throw new Error('카테고리 코드 조회에 실패했습니다.');
        }
        return response.json();
      })
      .then(data => {
        return data.categoryCode1;
      });
}
...
function categoryView(cate){
  console.log("***********************************");
  console.log(cate);
  let categoryNum = getCategoryCode1();
  let list = '';

  cate.forEach(c => {

    if(c.categoryCode1 == categoryNum) {
      list += `
                
       <li class="_next selected">
         (대분류) ${c.categoryName} &gt;
...
function showCategoryList(list){
  ...
  list.forEach(c => {
    if(c.categoryCode2 == categoryCode2) {
      list += `
                
       <li class="_next selected">
         (대분류) ${c.categoryName} &gt;
        <input type="hidden" name="categoryCode2" class="_category_no" value="${c.categoryCode2}">
       </li>
    `;
    }
